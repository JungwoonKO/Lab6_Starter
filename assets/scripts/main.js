...
function addRecipesToDocument(recipes) {
  ...
  for(let i = 0; i < recipes.length; i++) {
    const newRecipes = document.createElement('recipe-card');
    newRecipes.data = recipes[i];
    theMain.appendChild(newRecipes);
  }
}
...
function initFormHandler() {
  ...
  //            values from the FormData object and insert them into recipeObjec
    let recipeObject = {
  ...
  const clearLocal = form.querySelectorAll('button')[1];
  // B11. TODO - Add a click event listener to clear local storage button
  clearLocal.addEventListener('click', clean);
